...
DeNCS is a Java CLI decompiler for KotOR (Knights of the Old Republic) NCS script files. It converts compiled `.ncs` bytecode back to readable `.nss` source. See `README.md` for full build/run/test documentation.
...
- **Package**: `mvn package` (produces fat JAR at `target/dencs-CLI-1.0.2.jar`)
- **CLI usage**: `java -jar target/dencs-CLI-1.0.2.jar -i <input.ncs> -o <output.nss> -g k1|k2`
...
- Round-trip tests (`DeNCSCLIRoundTripTest`) clone the `Vanilla_KOTOR_Script_Source` GitHub repo into `test-work/` on first run. This directory is gitignored.
